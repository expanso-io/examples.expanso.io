...
sidebar_position: 7
description: Inspect an authored log-filtering policy and severity scores.
...
uniqueTasks: [inspect-production-pipeline-step-4-filter-score-logs]
claimIds: []
...
Not all logs carry the same diagnostic value. This step applies explicit filtering and scoring rules.
...
1.  **Test Dropped Log (DEBUG):**
    ...
    _Result: No output (log dropped)._
...
2.  **Test Kept Log (ERROR):**
    ...
    ```
    _Result: Log output with score=8._

## Next Steps

We've reduced noise. Now we must ensure we don't leak sensitive user data.

👉 [**Step 5: Redact Sensitive Data**](/log-processing/production-pipeline/step-5-redact-sensitive-data)
