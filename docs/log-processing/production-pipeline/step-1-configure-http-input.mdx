...
sidebar_label: 1. Configure HTTP Input
...
2.  **Deploy:**
    ```bash
    expanso job deploy production-pipeline-step-1.yaml
    ```

3.  **Test Access (Success):**
    ```bash
...
4.  **Test Access (Failure - Wrong Key):**
    ```bash
    curl -X POST http://localhost:8080/logs/ingest \
...
👉 [**Step 2: Parse & Validate Logs**](./step-2-parse-validate-logs)
