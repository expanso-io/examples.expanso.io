...
### Why 6 stages instead of just parse → output?
Each stage serves a specific production need: validation (catch bad data), enrichment (observability), filtering (cost reduction), PII redaction (compliance), fan-out (reliability). Skipping stages creates gaps in reliability, compliance, or cost efficiency.

### Can I simplify this for development?
Yes! For dev/staging, skip filtering (stage 4) and use single output (skip stage 6). Keep validation (stage 2) and PII redaction (stage 5) even in dev to catch issues early.

### How do I monitor this pipeline?
Each stage emits metrics: validation failures, filter drop rate, PII fields redacted, destination delivery rates. Use Prometheus + Grafana to visualize pipeline health in real-time.

### What's the throughput?
This 6-stage pipeline handles 100k-1M logs/second on a single edge node. Bottlenecks are usually output destinations (Elasticsearch write rate, S3 upload bandwidth), not pipeline processing.
