...
- ✅ Expanso edge platform installed ([Installation Guide](https://docs.expanso.io/install))
...
[ -d /var/log/app ] && echo "✅ /var/log/app exists" || echo "❌ /var/log/app missing"
[ -d /var/log/expanso ] && echo "✅ /var/log/expanso exists" || echo "❌ /var/log/expanso missing"
echo

# Check permissions
echo "🔑 Permissions:"
[ -w /var/log/app ] && echo "✅ Write access to /var/log/app" || echo "❌ No write access to /var/log/app"
[ -w /var/log/expanso ] && echo "✅ Write access to /var/log/expanso" || echo "❌ No write access to /var/log/expanso"
echo

# Check log files
echo "📄 Log Files:"
[ -f /var/log/app/application.log ] && echo "✅ Sample JSON logs present ($(wc -l < /var/log/app/application.log) lines)" || echo "❌ No JSON logs"
[ -f /var/log/app/legacy.log ] && echo "✅ Sample plain text logs present ($(wc -l < /var/log/app/legacy.log) lines)" || echo "❌ No plain text logs"
echo

# Check environment
echo "🌍 Environment:"
[ ! -z "$NODE_ID" ] && echo "✅ NODE_ID set to: $NODE_ID" || echo "❌ NODE_ID not set"
echo

# Check Expanso platform
echo "🚀 Expanso Platform:"
if command -v expanso > /dev/null; then
  echo "✅ Expanso CLI available"
  if expanso version > /dev/null 2>&1; then
    echo "✅ Expanso platform responsive"
  else
    echo "❌ Expanso platform not responding"
  fi
else
...
📁 Directory Setup:
✅ /var/log/app exists
✅ /var/log/expanso exists

🔑 Permissions:
✅ Write access to /var/log/app
✅ Write access to /var/log/expanso

📄 Log Files:
✅ Sample JSON logs present (9 lines)
✅ Sample plain text logs present (7 lines)

🌍 Environment:
✅ NODE_ID set to: edge-001

🚀 Expanso Platform:
✅ Expanso CLI available
✅ Expanso platform responsive
...
**Symptom:** Cannot create directories in `/var/log/`

**Solution:**
```bash
# Option 1: Use sudo and fix ownership
sudo mkdir -p /var/log/app /var/log/expanso
sudo chown $USER:$USER /var/log/app /var/log/expanso

# Option 2: Use alternative directory (for development)
mkdir -p ~/logs/app ~/logs/expanso
export LOG_BASE_PATH="$HOME/logs"
# Then use $HOME/logs/app/* in pipeline configurations
```

### Issue: Expanso CLI Not Found

**Symptom:** `command not found: expanso`

**Solution:**
```bash
# Check if Expanso is installed but not in PATH
find /usr/local /opt /home -name expanso -type f 2>/dev/null

# If found, add to PATH
export PATH="$PATH:/path/to/expanso/bin"

# If not found, install Expanso platform
curl -sSL https://install.expanso.io | bash
```

### Issue: No Log Output From Pipeline

**Symptom:** Pipeline runs but no output appears
...
expanso logs shell-filter-severity --errors
...
**Solutions:**
1. **File not readable:** Ensure log files exist and are readable
2. **Wrong file pattern:** Verify `/var/log/app/*.log` matches your files
3. **Pipeline not started:** Restart with `expanso run shell-filter-severity.yaml`
