...
uniqueTasks: [inspect-filter-severity-step-4-advanced-patterns]
claimIds: []
...
Once you have mastered the basics of parsing, filtering, and routing, you can combine these techniques into more advanced pipelines.
...
Sometimes you want to send a message to _multiple_ destinations. For example, you might want to send an `ERROR` log to both a file for long-term storage and to the console for immediate visibility. This is done by using a `broker` with the `fan_out` pattern inside a `switch` case.
...
This configuration attempts the outputs in order. Its delivery and loss behavior depends on each output, retry, timeout, acknowledgement, and restart contract; none is verified here.
