...
- ✅ At least one edge node connected to your orchestrator
...
if [ $errors -eq 0 ]; then
  ...
  echo ""
  echo "❌ $errors variable(s) missing. Review Step 1 and Step 2."
  exit 1
fi
EOF
...
```bash
expanso job logs pii-shell-pipeline --tail 50
```
