...
```json
{
  ...
  "location": {
    "latitude": 37.7749,                         // 🔴 Precise tracking
    "longitude": -122.4194,                      // 🔴 Precise tracking
    "city": "San Francisco",
    "state": "California",
    "country": "USA"
  },
...
```json
{
  ...
  "timestamp": "2024-01-15T10:30:00Z",
  "event_type": "purchase",
  // "user_name" removed                         ✅ Deleted
  // "email" removed                             ✅ Deleted
  // "ip_address" removed                        ✅ Deleted
  "user_id": "user_8f3e7a9c2d1b",                // ✅ Pseudonymous
  "email_hash": "a7b3c2d1e4f5...",               // ✅ Hashed
  "email_domain": "example.com",                 // ✅ Extracted
  "ip_hash": "8f3e7a9c2d1b4e6f...",              // ✅ Hashed
  "payment_method": {
    "type": "credit_card",                       // ✅ Kept
    "last_four": "9010"                          // ✅ Kept
    // "full_number" removed                     ✅ Deleted
    // "expiry" removed                          ✅ Deleted
  },
  "location": {
    // "latitude" removed                        ✅ Deleted
    // "longitude" removed                       ✅ Deleted
    "city": "San Francisco",                     // ✅ Kept
    "state": "California",                       // ✅ Kept
    "country": "USA"                             // ✅ Kept
  },
  "purchase_amount": 49.99,                      // ✅ Kept
  "currency": "USD"                              // ✅ Kept
}
```
...
# - user_name → user_id (pseudonymized)
# - email → email_hash + email_domain
# - ip_address → ip_hash
...
**Challenge:** Rotating salts changes hashes → breaks user tracking
...
- ✅ All PII fields removed from output
- ✅ Same input → same output (deterministic hashing)
- ✅ Salts are NOT in YAML (using env vars or secrets)
- ✅ Output destination is correct
- ✅ Error handling works (test with malformed events)
- ✅ Performance meets requirements (load test)
...
✅ **Count unique users** (via email_hash or user_id)
✅ **Track user journeys** (via user_id across sessions)
✅ **Detect abuse** (via ip_hash rate limiting)
✅ **Analyze by organization** (via email_domain)
✅ **Regional analytics** (via city, state, country)
✅ **Payment type trends** (via payment_method.type)
✅ **Fraud detection** (via payment_method.last_four)

You cannot:
❌ **Identify individuals** (names, emails, IPs removed)
❌ **Contact users** (emails hashed - need separate mapping)
❌ **Precise tracking** (GPS coords removed)
❌ **Reverse any hashes** (one-way transformation)
...
- [📥 Complete Pipeline YAML](https://examples.expanso.io/files/data-security/remove-pii-complete.yaml)
- [📥 Sample Data JSON](https://examples.expanso.io/files/data-security/remove-pii/sample-data.json)
- [📥 Incremental Pipelines (Steps 1-5)](https://examples.expanso.io/files/data-security/remove-pii/)
...
- [**Schema Enforcement**](../../data-security/enforce-schema) - Validate data structure before processing
