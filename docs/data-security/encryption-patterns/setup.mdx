...
- ✅ Basic familiarity with symmetric encryption (AES-256)
- ✅ Access to a key management system (AWS KMS, HashiCorp Vault, or environment variables)
...
if command -v expanso &> /dev/null; then
    echo "✅ Expanso CLI installed: $(expanso version)"
else
...
for var in CARD_ENCRYPTION_KEY PII_ENCRYPTION_KEY ADDRESS_ENCRYPTION_KEY TEMPORAL_ENCRYPTION_KEY; do
    if [ ${#!var} -ne 64 ]; then
        echo "❌ $var: invalid length ${#!var} (expected 64)"
        exit 1
    fi
done
...
if [ -f "sample-data.json" ]; then
    ...
    if jq empty sample-data.json 2>/dev/null; then
        echo "✅ Sample data is valid JSON"
    else
        echo "❌ Sample data is invalid JSON"
        exit 1
    fi
else
...
if [ -f "shell-output.jsonl" ]; then
    echo "✅ Shell pipeline executed successfully"
    
    # Validate output format
    if jq empty shell-output.jsonl 2>/dev/null; then
        echo "✅ Shell pipeline output is valid JSON"
        
        # Check encryption metadata was added
        if jq '.encryption_metadata.encryption_ready' shell-output.jsonl | grep -q true; then
            echo "✅ Encryption metadata added correctly"
        else
            echo "❌ Encryption metadata missing or invalid"
            exit 1
        fi
    else
        echo "❌ Shell pipeline output is invalid JSON" 
        exit 1
    fi
else
    echo "❌ Shell pipeline did not produce output"
    exit 1
fi
...
if [ -f "encryption-test.jsonl" ]; then
    ...
    if jq '.encryption_test.success' encryption-test.jsonl | grep -q true; then
        echo "✅ Encryption round-trip test passed"
    else
        echo "❌ Encryption round-trip test failed"
        exit 1
    fi
else
    echo "❌ Encryption test did not complete"
    exit 1
fi

echo ""
echo "🎉 Setup validation complete! Your environment is ready for field-level encryption."
echo ""
echo "Next steps:"
...
### Issue: "expanso command not found"

**Solution:** Install Expanso Edge Runtime:
```bash
# Download and install Expanso
curl -fsSL https://install.expanso.io | bash

# Add to PATH
export PATH="$HOME/.expanso/bin:$PATH"

# Verify installation
expanso version
```

### Issue: "Key length is not 64 characters"

**Solution:** Regenerate keys with proper OpenSSL command:
```bash
# Ensure OpenSSL generates 32 bytes (64 hex chars)
openssl rand -hex 32 | wc -c  # Should output 65 (64 chars + newline)

# If you get the wrong length:
CARD_ENCRYPTION_KEY=$(openssl rand -hex 32 | tr -d '\n')
echo "Length: ${#CARD_ENCRYPTION_KEY}"  # Should be exactly 64
```

### Issue: "JSON parsing error in sample data"

**Solution:** Validate and fix JSON formatting:
```bash
# Check for JSON syntax errors
cat sample-data.json | jq empty

# If errors, recreate the file:
rm sample-data.json
# Re-run Step 4 to recreate sample-data.json
```

### Issue: "Shell pipeline fails to start"

**Solution:** Check job logs for detailed errors:
```bash
# Get detailed job status
expanso job status shell-encryption-patterns --verbose

# Check job logs
expanso job logs shell-encryption-patterns

# Common issue: File path not found
ls -la sample-data.json  # Verify file exists
...
env | grep -E "(CARD_|PII_|ADDRESS_|TEMPORAL_)ENCRYPTION_KEY"

# Test key format manually
echo "Test" | openssl enc -aes-256-gcm -K $CARD_ENCRYPTION_KEY -iv $(openssl rand -hex 12) | base64
```
