...
uniqueTasks: [review-encryption-patterns-step-3-encrypt-addresses]
claimIds: []
...
2.  **Add the Address Encryption Logic:** Add the logic for the `billing_address` object to the _bottom_ of the existing `mapping` processor.
    ...
    _Note: This step uses a separate `ADDRESS_ENCRYPTION_KEY`. Ensure you have set it as an environment variable._
