...
uniqueTasks: [review-encryption-patterns-step-4-encrypt-dates]
claimIds: []
...
2.  **Add the Date Encryption Logic:** Add the logic for the `date_of_birth` field to the _bottom_ of the existing `mapping` processor.
    ...
    _Note: This step uses a separate `TEMPORAL_ENCRYPTION_KEY`. Ensure you have set it as an environment variable._
