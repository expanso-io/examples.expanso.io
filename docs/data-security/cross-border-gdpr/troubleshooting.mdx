...
# Find failed executions, then inspect the reported execution ID
expanso-cli job executions eu-cross-border-compliance --state failed --limit 5
expanso-cli execution describe <failed-execution-id>
...
**Symptoms:** Compliance risk—data leaving EU before anonymization.
...
### How do I check whether the configured transformations ran?

**Generate a technical check report:**
...
This query checks the example's own flag. It does not inspect every field, establish anonymous-data status, or prove that a transfer complies with GDPR.
...
3. **Check the [Complete Pipeline](/data-security/cross-border-gdpr/complete-pipeline)** for reference

4. **Review GDPR guidance:** [ICO UK GDPR guidance and resources](https://ico.org.uk/for-organisations/uk-gdpr-guidance-and-resources/)
