...
sidebar_position: 9
description: Reference cross-border pipeline demonstrating field removal, pseudonymization, generalization, and validation
keywords: [complete, gdpr, cross-border, deployment]
contentArchetype: reference
executionStatus: architecture-only
operationalEvidence: not-assessed
localNavigation: true
uniqueTasks: [review-cross-border-gdpr-complete-pipeline]
claimIds: []
...
This reference pipeline combines six technical data-protection steps for a cross-border transfer scenario:
...
6. **Validate configured fields** - Technical gate before output

:::caution Legal scope
The configuration does not determine legal status or make the output anonymous. Stable salted hashes are pseudonymous and can remain personal data. Use the output after privacy and legal review of identifiability, transfer mechanisms, access, retention, and the surrounding system.
:::
...
EU Database ──→ [Anonymization Pipeline] ──→ Global BigQuery (anonymized)
                        ...
                        │
                        └──→ Audit Log (transformation records)
```

## Full Configuration

<CodeBlock language="yaml" title="cross-border-gdpr.yaml">
  {pipelineYaml}
</CodeBlock>
...
**Expected Output (transformed and pseudonymized):**
...
## Illustrative deployment command

### EU-scoped example deployment

To illustrate a location selector, deploy to an EU-labeled example node:

```bash
# Deploy with region selector
expanso-cli job deploy cross-border-gdpr.yaml \
  --selector region=eu \
  --selector data_policy=review-required

# Verify deployment location
expanso-cli job describe eu-cross-border-minimization
```
...
name: eu-cross-border-minimization
...
<a
  href="/files/data-security/cross-border-gdpr.yaml"
  download
...
- [Troubleshooting](/data-security/cross-border-gdpr/troubleshooting) - Common issues and solutions
- [Remove PII](/data-security/remove-pii) - General PII removal patterns
