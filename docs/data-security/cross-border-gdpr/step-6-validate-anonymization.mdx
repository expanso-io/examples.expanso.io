...
sidebar_position: 8
description: Verify that the example's configured direct-identifier fields were transformed before output
keywords: [validation, transformation, gate, verification, audit]
contentArchetype: step
executionStatus: architecture-only
operationalEvidence: not-assessed
localNavigation: true
uniqueTasks: [review-cross-border-gdpr-step-6-validate-anonymization]
claimIds: []
...
Before output, verify that the direct-identifier fields named by this example were removed or transformed. This technical check does not prove that no personal data remains or that legal obligations have been met.

## The Goal

- Check that no PII fields remain in the record
- Throw an error (reject the record) if validation fails
- Add a transformation-check record with a timestamp
- Document which fields were removed/hashed

## Why This Matters

**Defense in Depth:** The gate catches the explicitly listed fields if an earlier transformation fails. It does not detect every identifier, linkage risk, or new schema field.

**Audit Context:** The record shows that the configured checks ran before output; it does not prove anonymous-data status.

**Fail-Safe:** Reject records that fail the configured checks; obtain legal review for transfer requirements.
...
pipeline:
  processors:
    ...
    - mapping: |
        ...
        # Add example transformation metadata
...
Record is rejected—does not proceed to output.
...
Generate daily summaries of the selected technical checks:
...
You've built all six technical steps in the example. Review the complete reference configuration:
