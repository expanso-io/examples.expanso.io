...
✅ **PCI-DSS data classification** - What constitutes cardholder data vs. sensitive authentication data  
✅ **Risk levels** - Which fields require immediate deletion vs. which can be processed  
✅ **Analytics impact** - How to preserve business value while achieving compliance  
✅ **Compliance boundaries** - Where PCI-DSS scope begins and ends in your data pipeline
...
{
  "payment_method": {
    "full_number": "4532-1234-5678-9010",  // ❌ Primary Account Number (PAN)
    "expiry": "12/25"                      // ❌ Expiration date
  }
}
```
...
{
  "payment_method": {
    "type": "credit_card",                // ✅ Payment method type
    "brand": "visa",                      // ✅ Card brand/network  
    "last_four": "9010"                   // ✅ Last 4 digits of PAN
  },
  "amount": 89.99,                        // ✅ Transaction amount
...
- ✅ **Preserved:** Transaction patterns, amounts, timing
- ❌ **Lost:** Ability to identify specific cards across transactions
- 🔄 **Alternative:** Use last 4 digits for limited card identification
...
**Analytics Impact of Removal:**
- ✅ **Preserved:** All transaction analytics remain intact
- ❌ **Lost:** Card lifecycle analytics (renewal patterns, expiry tracking)
- 🔄 **Alternative:** Derive expiry year only for lifecycle analysis
...
**Analytics Impact of Removal:**
- ✅ **Preserved:** All other transaction data unchanged
- ❌ **Lost:** Nothing - CVV has no analytics value
- 🔄 **Alternative:** None needed - CVV is purely for authentication
...
```json
{
  "event_type": "purchase",              // 🟢 Safe - Event classification
  "transaction_id": "txn_001",           // 🟢 Safe - Transaction identifier
  "timestamp": "2024-12-15T14:30:00Z",   // 🟢 Safe - Business data
  "amount": 89.99,                       // 🟢 Safe - Business data  
  "currency": "USD",                     // 🟢 Safe - Business data
  "merchant": {                          // 🟢 Safe - Business data
    "id": "merchant_123",
    "name": "Coffee Shop Downtown",
    "category": "food_beverage"
  },
  "payment_method": {
    "full_number": "4532-1234-5678-9010", // 🔴 Delete - Cardholder data
    "expiry": "12/25",                     // 🔴 Delete - Cardholder data
    "cvv": "123",                          // 🔴 Delete - Sensitive auth data
    "type": "credit_card",                 // 🟢 Safe - Payment analytics
    "brand": "visa",                       // 🟢 Safe - Payment analytics
    "last_four": "9010"                    // 🟢 Safe - Fraud detection
  },
  "customer": {                          // 🟡 Moderate - Not payment PII
    "id": "cust_456",                     // 🟢 Safe - Customer analytics
    "email": "sarah@example.com"          // 🟡 Moderate - Personal data (GDPR)
  }
}
```

**Summary:**
- **🔴 Must delete:** 3 fields (full_number, expiry, cvv)
- **🟢 Safe to keep:** 9 fields (business + analytics data)
- **🟡 Moderate risk:** 1 field (email - GDPR concern, not PCI-DSS)
...
```json
{
  "order_id": "ord_12345",               // 🟢 Safe - Business identifier
  "payment": {
    "pan": "4111111111111111",           // 🔴 Delete - Cardholder data
    "exp_month": 12,                     // 🔴 Delete - Cardholder data
    "exp_year": 2025,                    // 🔴 Delete - Cardholder data  
    "cvc": "123",                        // 🔴 Delete - Sensitive auth data
    "cardholder_name": "John Smith",     // 🔴 Delete - Cardholder data
    "billing_address": {                 // 🟡 Moderate - May be cardholder data
      "street": "123 Main St",           // 🟡 Depends on use case
      "zip": "90210"                     // 🟢 Safe - Geographic analytics
    }
  },
  "fraud_signals": {
    "ip_address": "192.168.1.1",        // 🟡 Moderate - Privacy data
    "user_agent": "Mozilla/5.0...",     // 🟢 Safe - Technical data
    "device_fingerprint": "abc123"      // 🟢 Safe - Fraud detection
  }
}
```
...
{
  "wallet_transaction": {
    "token": "dpan_1234567890123456",     // 🟢 Safe - Payment token (not real PAN)
    "cryptogram": "ABC123DEF456",        // 🟢 Safe - Transaction cryptogram
    "original_pan": "4111111111111111",  // 🔴 Delete - If stored, becomes cardholder data
    "last_four": "1111"                  // 🟢 Safe - Display/analytics only
  }
}
```
...
{
  "subscription": {
    "card_fingerprint": "fp_abc123",     // 🟢 Safe - Uniqueness without PAN
    "payment_method_id": "pm_xyz789",    // 🟢 Safe - Internal identifier
    "last_four": "1234",                 // 🟢 Safe - Customer display
    "brand": "visa",                     // 🟢 Safe - Analytics
    "exp_month": 12,                     // 🔴 Delete - Cardholder data
    "exp_year": 2025,                    // 🔴 Delete - Cardholder data
    "funding": "credit"                  // 🟢 Safe - Payment method analytics
  }
}
```
...
```
┌─────────────────┐    ┌───────────────────────────────┐
│ PCI-DSS Scope   │    │        Non-PCI Environment    │
│ ┌─────────────┐ │    │  ┌─────────────────────────┐ │
│ │   Payment   │ │    │  │    Data Storage         │ │
│ │  Processing │─┼────┼─▶│  (Safe fields only)     │ │
│ │  (Delete)   │ │    │  │                         │ │
│ └─────────────┘ │    │  └─────────────────────────┘ │
└─────────────────┘    │                               │
                       │  ┌─────────────────────────┐ │
...
**Answers:**
1. 🟢 Safe - Network token, not real PAN
2. 🟢 Safe - Transaction data, high analytics value
3. 🟢 Safe - Masked PAN shows only last 4 digits
4. 🟢 Safe - Authorization reference, not cardholder data  
5. 🟢 Safe - Geographic analytics data
6. 🔴 Delete - CVV/CVC, sensitive authentication data
7. 🟢 Safe - Internal processor identifier
...
✅ **High-risk fields identified:** full_number, expiry, cvv - must be deleted  
✅ **Safe fields identified:** last_four, type, brand, amounts - keep for analytics  
✅ **Compliance boundaries understood:** PCI-DSS scope limited to processing layer  
✅ **Analytics impact assessed:** Minimal loss, significant compliance gain
...
<div style={{display: 'flex', gap: '1.5rem', marginTop: '2rem', marginBottom: '3rem', flexWrap: 'wrap', justifyContent: 'flex-start'}}>
  <a href="./step-2-delete-card-numbers" className="button button--primary button--lg" style={{display: 'inline-flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', borderRadius: '8px', padding: '1rem 2rem', fontWeight: '600', minWidth: '240px', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', cursor: 'pointer', transition: 'all 0.2s ease'}}>
