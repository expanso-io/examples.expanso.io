...
- ✅ Basic familiarity with payment processing
...
✅ Connected to https://api.expanso.io
```
...
- ✅ **Safe fields:** transaction_id, amount, currency, merchant info, last_four
- ❌ **High-risk PII:** full_number, expiry, cvv (violate PCI-DSS)
- 🟡 **Moderate PII:** customer email (not payment-specific)
...
- ❌ `full_number` - Primary Account Number (PAN), highest risk
- ❌ `expiry` - Cardholder data, moderate risk  
- ❌ `cvv` - Sensitive authentication data, highest risk
- ✅ `type`, `brand`, `last_four` - Safe for analytics
...
echo "🧪 Testing pipeline: $PIPELINE_NAME"
echo "📁 Using data file: $DATA_FILE"

# Get pipeline endpoint
echo "📡 Getting pipeline endpoint..."
ENDPOINT=$(expanso job describe $PIPELINE_NAME | grep "endpoint" | cut -d'"' -f4)

if [ -z "$ENDPOINT" ]; then
  echo "❌ Could not find endpoint for pipeline $PIPELINE_NAME"
  echo "💡 Make sure the pipeline is deployed and running"
  exit 1
fi

echo "🔗 Endpoint: $ENDPOINT/events/payment"

# Send test data
echo "📤 Sending test payment events..."
...
# Test the script
echo "🧪 Testing the shell pipeline..."
...
**Symptom:** `expanso auth status` shows "Not authenticated"
...
**Symptom:** `expanso job deploy` returns errors

**Diagnosis:**
```bash
# Check deployment status
expanso job describe shell-payment-pipeline

# Check for syntax errors
yamllint shell-payment-pipeline.yaml
```
...
python3 -c "import yaml; yaml.safe_load(open('shell-payment-pipeline.yaml'))"
```

**2. Name Conflicts**
```bash
# Delete existing pipeline if needed
expanso job delete shell-payment-pipeline

# Redeploy
expanso job deploy shell-payment-pipeline.yaml
```

### Issue: No Pipeline Logs

**Symptom:** `expanso job logs` shows no output

**Diagnosis:**
```bash
# Check pipeline status
expanso job status shell-payment-pipeline

# Check if events are being sent
curl -v -X POST $PIPELINE_URL/events/payment \
  -H "Content-Type: application/json" \
  -d '{"test": "event"}'
```
...
expanso job logs shell-payment-pipeline --tail 50
```

### Issue: Sample Data Invalid

**Symptom:** JSON parsing errors when sending test data

**Solution:**
```bash
# Validate JSON structure
cat sample-payments.json | jq '.'

# Fix any syntax errors and recreate the file
# Each JSON object should be on a separate line (JSONL format)
```
...
- [ ] ✅ Expanso CLI installed and authenticated
- [ ] ✅ Sample payment data created (`sample-payments.json`)
- [ ] ✅ Shell pipeline deployed and running
- [ ] ✅ Test script working (`test-payment-pipeline.sh`)
- [ ] ✅ Environment variables configured (`.env`)
- [ ] ✅ Can send events and see logs
...
✅ **Environment configured** with Expanso CLI and authentication  
✅ **Sample payment data** created with realistic PII scenarios  
✅ **Shell pipeline** deployed and tested successfully  
✅ **Testing tools** created for iterative development  
✅ **Ready to learn** payment PII identification and removal techniques
