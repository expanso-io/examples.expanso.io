...
- ✅ Basic familiarity with AES-256-GCM encryption
...
**Symptom:** `openssl rand` command not found or fails
...
### Issue: Vault integration fails
**Symptom:** `vault: command not found` or authentication errors
...
**Symptom:** `curl` commands fail with certificate errors

**Solutions:**

**1. Generate self-signed certificates for testing:**
```bash
sudo mkdir -p /etc/expanso/certs
sudo openssl req -x509 -newkey rsa:4096 -keyout /etc/expanso/certs/server.key -out /etc/expanso/certs/server.crt -days 365 -nodes -subj "/CN=localhost"
sudo openssl req -x509 -newkey rsa:4096 -keyout /etc/expanso/certs/client.key -out /etc/expanso/certs/client.crt -days 365 -nodes -subj "/CN=client"
```

**2. Use system CA bundle:**
```bash
# Use system certificates instead
curl --cacert /etc/ssl/certs/ca-certificates.crt ...
```

### Issue: Environment variables not persisting
**Symptom:** Variables lost between shell sessions

**Solution:**
```bash
# Add to shell profile for persistence
cat >> ~/.bashrc << 'EOF'
# Expanso encryption environment
export CARD_ENCRYPTION_KEY="your-key-here"
export PII_ENCRYPTION_KEY="your-key-here"  
export ADDRESS_ENCRYPTION_KEY="your-key-here"
