...
- ✅ Expanso platform installed and running ([Installation Guide](https://docs.expanso.io/getting-started/installation))
- ✅ At least one edge node connected to orchestrator
- ✅ Basic familiarity with JSON Schema format
...
### Issue: Edge Nodes Not Listed

**Symptom:** `expanso node list` returns empty or nodes missing labels
...
expanso node describe <NODE_ID>

# Add missing labels
expanso node label <NODE_ID> role=sensor-collector validation=required
```

### Issue: Shell Pipeline Not Starting

**Symptom:** Pipeline status shows "failed" or "pending"

**Solution:**
```bash
# Check pipeline logs
expanso job logs shell-sensor-validation

# Verify node selector matches labels
expanso node list --show-labels | grep validation

# Check for port conflicts
expanso node exec <NODE_ID> -- netstat -ln | grep 8080
```

### Issue: Schema Directory Permission Denied

**Symptom:** Cannot create schema directory on edge nodes

**Solution:**
```bash
# Check current user permissions
expanso node exec <NODE_ID> -- whoami

# Use sudo if needed
expanso node exec <NODE_ID> -- sudo mkdir -p /etc/expanso/schemas
expanso node exec <NODE_ID> -- sudo chown expanso:expanso /etc/expanso/schemas
```
