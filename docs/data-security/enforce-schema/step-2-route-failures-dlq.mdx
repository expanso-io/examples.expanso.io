...
uniqueTasks: [review-enforce-schema-step-2-route-failures-dlq]
claimIds: []
...
5.  **Verify:**
    ...
    - The **invalid** message will _not_ be rejected. Instead, it will be written to the `validation_dlq.jsonl` file, along with a `validation_error` field that explains why it failed.
