...
Build a pipeline that removes PII from streaming data at the edge—before it reaches your cloud systems. This reduces compliance scope, protects user privacy, and maintains data utility for analytics.
...
{
  "event_type": "purchase",
  "user_name": "Sarah Johnson",        // ❌ Direct PII
  "email": "sarah@example.com",        // ❌ Direct PII
  "ip_address": "192.168.1.100",       // ❌ GDPR considers this PII
  "payment_method": {
    "full_number": "4532-1234-5678-9010"  // ❌ High-risk PCI data
  },
  "location": {
    "latitude": 37.7749,               // ❌ Precise location
    "longitude": -122.4194,
...
1. **Delete** → Credit cards, precise coordinates (high-risk, no analytics value)
2. **Hash** → Emails, IPs (preserve uniqueness for counting)
3. **Pseudonymize** → User names (consistent IDs for tracking)
4. **Generalize** → Location (keep city/country, remove coordinates)
...
```
✓ Job 'pii-removal-pipeline' created
✓ Evaluation triggered
✓ Scheduling to matching nodes...
✓ Deployed to 3 nodes (edge-node-001, edge-node-002, edge-node-003)
```
...
✅ Event type and page URLs (user behavior)
✅ Timestamps (time-series analysis)
✅ Campaign and A/B test groups (marketing analytics)
✅ City and country (regional trends)
✅ Session IDs (user journey tracking)
✅ Hashed user IDs (unique user counting)
✅ Email domains (organizational analysis)
✅ User agent (device/browser analysis)

**What's Removed (PII):**

❌ Personal names
❌ Email addresses (but domain preserved)
❌ IP addresses (but hash preserved for abuse detection)
❌ Precise locations
...
Daily bandwidth: 2.5 KB × 1000 × 86400 = 216 GB/day
...
Events per second: 1000
Daily bandwidth: 1.8 KB × 1000 × 86400 = 155 GB/day
...
You can go from email → hash, but never hash → email.
