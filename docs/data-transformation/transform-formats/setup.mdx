...
- ✅ Docker installed for schema registry container
- ✅ Cloud storage access (AWS S3, GCS, or Azure Blob) for Parquet examples
- ✅ Basic familiarity with data serialization formats (JSON, Avro, Parquet, Protobuf)
...
```bash
docker stop format-schema-registry
docker rm format-schema-registry
# Run the docker command from Step 2 again
```
...
```bash
# AWS - Add appropriate bucket policy
aws s3api get-bucket-policy --bucket $CLOUD_STORAGE_BUCKET
...
**Symptom:** `Pipeline failed to start` or `Invalid configuration`
...
```bash
# Check YAML syntax
python -c "import yaml; yaml.safe_load(open('shell-format-transform.yaml'))"
```
...
**Symptom:** All formats detected as 'unknown'

**Solutions:**

**1. Verify Content-Type Headers**
```bash
# Test with explicit headers
curl -v -X POST http://localhost:8080/transform \
  -H "Content-Type: application/json" \
  -d '{"test": true}'
```
