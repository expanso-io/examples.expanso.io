...
uniqueTasks: [inspect-aggregate-time-windows-step-1-tumbling-windows]
claimIds: []
...
2.  **Inspect the intended output:** The configuration emits a summary after its ten-second period for each `sensor_id` represented in that window.

## Verification

The authored output is a stream of summary objects for `sensor-0` and `sensor-1`. This page does not record an executed result.
