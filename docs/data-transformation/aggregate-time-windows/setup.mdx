...
### Issue: "File not found" when processing sample data

**Symptom:** Pipeline fails with file reading errors

**Solution:** Verify file paths and permissions
```bash
# Check file exists and is readable
ls -la sensor-data.jsonl
file sensor-data.jsonl

# Fix permissions if needed
chmod 644 sensor-data.jsonl
```

### Issue: High memory usage during cache testing

**Symptom:** System becomes unresponsive or OOM errors
...
resources:
  caches:
    window_cache:
      memory:
        max_items: 10000  # Reduce from 100000
```
...
resources:
  caches:
    window_cache:
      memory:
        default_ttl: "30s"  # Reduce from 90s
```

**3. Sample events**
```yaml
# Process only 10% of events, multiply results by 10
- sampling:
    percentage: 10.0
```

### Issue: Time buckets show inconsistent grouping

**Symptom:** Uneven event distribution across time buckets

**Diagnosis:**
```bash
# Check timestamp parsing
grep '"timestamp"' shell-output.jsonl | head -5
grep '"time_bucket"' shell-output.jsonl | head -5

# Look for parsing errors
expanso logs shell-aggregate-time-windows | grep -i error
```

**Solution:** Verify timestamp format matches parse pattern
```yaml
# Ensure timestamp format matches your data
let parsed_time = this.timestamp.parse_timestamp("2006-01-02T15:04:05.000Z")  # Adjust format
```

### Issue: Environment variables not loaded

**Symptom:** Pipeline fails to connect to analytics endpoint

**Solution:** Verify environment configuration
```bash
# Check environment variables are set
env | grep -E "(ANALYTICS_ENDPOINT|ANALYTICS_API_KEY|BUFFER_PATH)"
