...
uniqueTasks: [inspect-aggregate-time-windows-step-2-sliding-windows]
claimIds: []
...
1.  **Generate Keys:** Instead of one `group_key`, you will generate an _array_ of keys for each message, one for each overlapping window it belongs to.
...
2.  **Modify the Processors:** Open `sliding-windows.yaml` and replace the entire `processors` array with the new logic below.
    ...
    _Note: You may also want to increase the `default_ttl` on the `window_cache` resource to `32s` to account for the longer window size._
