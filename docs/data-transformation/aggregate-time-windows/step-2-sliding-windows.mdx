...
3.  **Deploy and Observe:**
    ```bash
    # Deploy the pipeline
    expanso job deploy sliding-windows.yaml
    ```
    Watch the logs. You will now see output messages appearing every 10 seconds (the slide interval), and the `event_count` for each will be larger, as it includes events from a 30-second window.
