...
for port in 7001 7002 7003; do
    if redis-cli -h redis-cluster -p $port ping > /dev/null 2>&1; then
        echo "✅ Redis node :$port responding"
    else
        echo "❌ Redis node :$port not responding"
    fi
done
...
# Formula: events_per_second × TTL_seconds × bytes_per_entry
...
class CacheAnalyzer:
    ...
    def suggest_optimizations(self):
        ...
        if frag_ratio > 1.5:
            print("⚠️  High memory fragmentation detected")
            print("   Suggestion: Consider MEMORY PURGE or restart Redis")
            
        # Check evictions
        evicted_keys = info.get('evicted_keys', 0)
        if evicted_keys > 0:
            print(f"⚠️  {evicted_keys} keys evicted")
        ...
        if total_keys > 0 and expired_keys / total_keys > 0.5:
            print("⚠️  High key expiration rate")
...
echo "⚠️  This procedure will reset all cache state and restart services"
...
if ! curl -f -s $HEALTH_URL > /dev/null; then
    ...
    if ! curl -f -s $HEALTH_URL > /dev/null; then
        ...
        curl -X POST "$SLACK_WEBHOOK" -d '{"text":"🚨 Deduplication pipeline restart failed - manual intervention required"}'
    fi
fi
