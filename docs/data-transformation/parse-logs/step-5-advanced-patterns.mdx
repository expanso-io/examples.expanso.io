...
uniqueTasks: [inspect-parse-logs-step-5-advanced-patterns]
claimIds: []
...
When a processor like `parse_json` or `grok` fails, it stops processing that message. Wrap parsing attempts in a `try/catch` block to route malformed input to the configured Dead Letter Queue (DLQ) and continue processing other messages.
