...
2.  **Deploy and Observe:**
    ```bash
    # Deploy the pipeline
    expanso job deploy json-parser.yaml
    ```
    Watch the logs. The `generate` input creates messages with a single `raw_log` field. The output, however, will be the structured JSON that was parsed from inside that string.
