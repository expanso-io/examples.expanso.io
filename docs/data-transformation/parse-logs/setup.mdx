...
- ✅ Basic familiarity with log formats (helpful but not required)
...
ls -la ~/expanso-logs/app/application.jsonl

# Check job logs for errors  
expanso job logs shell-log-parser --level error
```
...
paths:
  - "/home/username/expanso-logs/app/*.jsonl"
```

**2. File permissions**
```bash
# Fix permissions
chmod 644 ~/expanso-logs/*/*.{jsonl,csv,log}
```

### Issue: Job deployment fails

**Diagnosis:**
```bash
# Check YAML syntax
expanso job validate shell-parser.yaml

# View detailed error
expanso job deploy shell-parser.yaml --verbose
```
...
cat -A shell-parser.yaml | grep "\t"
```

**2. Invalid configuration**  
```bash
# Use expanso config template
expanso job init log-parser --template file-input
```
...
ls -la ~/expanso-logs/output/

# Check job output configuration
expanso job describe shell-log-parser
```
...
output:
  file:
    path: /home/username/expanso-logs/output/shell-output.jsonl
```
