...
**Symptom:** 
```
Error: cannot write to /tmp/timestamp-input.jsonl: permission denied
```
...
```bash
ls -la /tmp/timestamp-input.jsonl
chmod 644 /tmp/timestamp-input.jsonl
```

2. **Use user directory instead:**
```bash
# Modify pipeline to use home directory
sed -i 's|/tmp/timestamp-input.jsonl|~/timestamp-samples/mixed-timestamps.jsonl|g' shell-timestamps.yaml
```
...
**Symptom:**
```
bash: expanso: command not found
```
...
### Issue: Timezone Database Outdated
...
1. **Update timezone data (Ubuntu/Debian):**
```bash
sudo apt-get update
sudo apt-get install -y tzdata
```

2. **Update timezone data (Alpine):**
```bash
sudo apk add tzdata
```

3. **Verify timezone update:**
```bash
zdump -v America/New_York | head -1
```

### Issue: Pipeline Deployment Fails

**Symptom:**
```
Error: failed to deploy pipeline: validation failed
```

**Diagnosis:**
```bash
# Check YAML syntax
yamllint shell-timestamps.yaml

# Validate pipeline configuration
expanso validate shell-timestamps.yaml

# Check pipeline logs
expanso logs --pipeline shell-timestamps --error
```
...
grep -E "^(input|pipeline|output):" shell-timestamps.yaml
```
