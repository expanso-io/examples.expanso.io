...
uniqueTasks: [review-fan-out-pattern-step-3-add-s3]
claimIds: []
...
2.  **Add the S3 Output:** To add S3, simply add an `aws_s3` block to the `outputs` array.
    ...
    The example adds a small `batching` configuration. Choose real batch thresholds from measured object size, arrival rate, upload behavior, and downstream query needs.
