...
uniqueTasks: [review-fan-out-pattern-step-4-add-elasticsearch]
claimIds: []
...
You will modify your `fan-out.yaml` file to send messages to a local file, a Kafka topic, an S3 bucket, _and_ an Elasticsearch index, all at the same time.
...
2.  **Add the Elasticsearch Output:** Add an `elasticsearch` block to the `outputs` array in your `broker` configuration.
    ...
    The example uses a daily index name (`sensor-events-YYYY-MM-DD`). The `id` field is derived from the event ID; verify update and duplicate behavior against the target service.
