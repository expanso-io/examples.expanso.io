...
- **Kafka:** Set up [Kafka](/getting-started/local-development#kafka) for fan-out message delivery
...
if [ -n "$S3_BUCKET" ]; then
  ...
  if aws s3 ls s3://$S3_BUCKET/ --region $AWS_REGION &>/dev/null; then
    ...
    echo "⚠️ S3 connectivity FAILED (optional)"
  fi
fi
...
if [ -n "$ES_ENDPOINT" ]; then
  ...
  if curl -s -X GET "$ES_ENDPOINT/_cluster/health" \
    ...
    echo "⚠️ Elasticsearch connectivity FAILED (optional)"
  fi
fi

# Test shell pipeline
echo "🏭 Testing shell pipeline..."
