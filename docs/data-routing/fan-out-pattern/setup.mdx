...
- ✅ Edge node connected to orchestrator
- ✅ Cloud service accounts (AWS, Kafka cluster, Elasticsearch cluster)
- ✅ Basic familiarity with YAML pipeline configuration
...
echo "🔍 Verifying fan-out setup prerequisites..."
...
if curl -s -X GET "$ES_ENDPOINT_1/_cluster/health" \
  ...
  echo "❌ Elasticsearch connectivity FAILED"
  exit 1
fi

# Test edge node
echo "🏭 Testing edge node pipeline..."
if expanso job status shell-fan-out-pipeline | grep -q "Running"; then
  echo "✅ Shell pipeline running OK"
else
  echo "❌ Shell pipeline not running"
  exit 1
fi
