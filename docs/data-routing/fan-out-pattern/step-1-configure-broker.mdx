...
uniqueTasks: [review-fan-out-pattern-step-1-configure-broker]
claimIds: []
...
You will convert a pipeline that writes to a single file into a pipeline that writes to _both_ a file and to your console (`stdout`) at the same time.
