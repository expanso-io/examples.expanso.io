...
uniqueTasks: [review-smart-buffering-step-4-prevent-starvation]
claimIds: []
...
If important messages keep arriving, lower-priority messages may remain queued. The example adds an age-based adjustment but does not assess eventual delivery.
...
After 2 hours, archive messages receive the same authored score as important messages. This changes sorting priority only; delivery still depends on queue and worker behavior that this example does not test.
...
This 3-hour-old archive message gets a +900 boost, bringing its score to 1000—equal to important messages.
...
4. Adds an age-based score adjustment for review
