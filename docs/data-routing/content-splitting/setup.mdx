...
- ✅ Expanso edge node installed ([Installation Guide](https://docs.expanso.io/installation))
- ✅ Destination services available (Kafka, S3, HTTP endpoints)
- ✅ Basic familiarity with YAML configuration ([Expanso Configuration](https://docs.expanso.io/configuration))

## Step 1: Configure Environment Variables

Set up environment variables for your destination services. This example uses Kafka and S3, but you can substitute any destinations:
...
Before proceeding, verify your destination services are reachable and properly configured:

```bash
# Test Kafka connectivity (if using Kafka)
...
**Symptom:** Pipeline fails to start with "bind: address already in use"
...
# In shell-splitting.yaml, change:
address: 0.0.0.0:8081  # Use different port
```

### Issue: Permission Denied Writing Log Files

**Symptom:** Pipeline starts but no log files appear

**Solution:**
```bash
# Create log directory with proper permissions
sudo mkdir -p /var/log/expanso
sudo chown $USER:$USER /var/log/expanso

# Or change to a user-writable directory
# In shell-splitting.yaml:
path: /tmp/shell-splitting-${!timestamp_unix()}.jsonl
```

### Issue: Environment Variables Not Set

**Symptom:** Pipeline fails when trying to connect to destinations
...
# Or add to pipeline config directly
# In your YAML file:
addresses: ["localhost:9092"]  # Instead of ${KAFKA_BROKERS}
```
