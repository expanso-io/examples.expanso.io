...
event_type: "business.custom_event"  # ✅ Will be classified
...
root.user_tier_score = match root.user_tier {
  "enterprise" | "platinum" | "vip" => 40  # ✅ Added VIP
...
{
  "event_type": "payment.failed",
  "amount": 50000,           # ✅ For business impact scoring
  "currency": "USD",
  "account_type": "enterprise"  # ✅ Additional context
}
```
...
{
  "event_id": "123",
  "region": "eu-west",  # ✅ Explicit region
  "user_data": "eu_user@example.com"
}
...
root.region = match root.region {
  ...
  "eu" | "europe" | "european-union" => "eu-west"  # ✅ Added
  "deutschland" | "germany" => "eu-central"        # ✅ Added
  # ... existing mappings ...
}
```
  ...
  switch:
    cases:
      - check: this.region == "eu-west"
        output:
          kafka:
            addresses: [${EU_KAFKA:eu-kafka.example.com:9092}]  # ✅ Use environment variable
...
root.event_family = if ["user", "auth", "session", "login"].contains(root.event_namespace) ||
                      ...
                      root.event_type.contains("signin") {  # ✅ Added signin
  "authentication"
```
...
body: |
  {
    "auth_event": {
      ...
      "user_context": {
        "user_id": this.user_id.or("unknown"),      # ✅ Provide default
        "source_ip": this.source_ip.or("unknown"),  # ✅ Provide default
        "user_agent": this.user_agent.or("unknown") # ✅ Provide default
      }
    }
  }
```
...
root.event_family = if ["payment", "transaction", "billing", "invoice", "charge"].contains(root.event_namespace) ||
                      ...
                      root.event_type.contains("purchase") ||     # ✅ Added
                      root.event_type.contains("checkout") ||     # ✅ Added  
                      this.amount.exists() {                      # ✅ Amount-based detection
  "payment"
```
...
root.priority_level = match root.priority_score {
  x if x >= 75 => "critical"  # ✅ Lowered from 85
  x if x >= 55 => "high"      # ✅ Lowered from 65
  # ... rest of thresholds
}
```
...
if [ "$LAG" -gt 10 ]; then
  echo "❌ Critical queue has high lag: $LAG"
else
  echo "✅ Critical queue lag normal: $LAG"
fi
```
...
- check: this.priority_level == "critical"
  output:
    kafka:
      ...
      # ✅ Absolutely no batching for critical events
      batching:
        count: 1
        period: 0s
...
root.user_tier_score = match root.user_tier {
  ...
  "premium" | "professional" | "pro" | "paid" => 35  # ✅ Added "paid"
  "plus" | "silver" | "advanced" => 25               # ✅ Added "advanced"
  # ... rest of tiers
}
```
...
- http_client:
    ...
    timeout: 5s  # ✅ Reduce from 30s default
...
batching:
  ...
  count: 100      # ✅ Not too large
  period: 10s     # ✅ Reasonable period
  byte_size: 1MB  # ✅ Size limit to prevent huge batches
```
...
batching:
  count: 500
  period: 60s
  byte_size: 5MB  # ✅ Add byte size limit
```
...
root.event_family = if root.event_namespace == "audit" ||
                      ...
                      this.actor.exists() ||           # ✅ Actor indicates audit
                      this.action.exists() ||          # ✅ Action indicates audit
                      this.admin_action.exists() {     # ✅ Admin action
  "audit"
}
```
    ...
    broker:
      ...
      outputs:
        ...
        - http_client:
            ...
            max_retries: 5  # ✅ More retries for audit
...
output:
  kafka:
    addresses: [${KAFKA_BROKERS}]  # ✅ Use environment variable
    topic: critical-queue
...
echo "📋 Content Router Diagnostic Report"
echo "================================="
echo "Timestamp: $(date)"
echo ""
...
- **Documentation:** [Complete Content Routing Guide](./index)
...
✅ Pipeline configuration (sanitized, no secrets)  
