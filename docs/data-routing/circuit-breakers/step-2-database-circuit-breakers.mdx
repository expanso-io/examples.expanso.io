...
uniqueTasks: [review-circuit-breakers-step-2-database-circuit-breakers]
claimIds: []
...
2.  **Stop the Database:** Now, simulate an outage.
...
2.  **Modify the Pipeline:** Open `database-circuit-breaker.yaml` and replace the `pipeline` section with the one below. This wraps the database call in a `try` block and adds a `catch` block for error handling.
    ...
    pipeline:
    processors: - try: # This processor will attempt to run. If it fails, # the 'catch' block is executed. - sql_select:
    driver: postgres
    data_source_name: $\{DB_CONNECTION_STRING\}
    query: |
    SELECT user_name, user_tier FROM users WHERE user_id = $1
    args_mapping: |
    root = [ this.user_id ] # These settings help fail faster
    timeout: 3s
    max_open_connections: 5

            # This mapping runs after a successful SQL query.
            - mapping: |
                root = this
            ...
            # This block runs when the 'sql_select' processor fails.
            - mapping: |
                root = this
