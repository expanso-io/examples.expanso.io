...
**Issue:** PostgreSQL container not accessible
...
# Check logs
docker logs circuit-test-db

# Restart if needed
docker restart circuit-test-db
sleep 10
```

### Pipeline Deployment Fails

**Issue:** Expanso CLI not connected

**Solution:**
```bash
# Check Expanso status
expanso status
