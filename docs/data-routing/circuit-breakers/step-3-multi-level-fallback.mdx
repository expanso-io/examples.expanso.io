...
uniqueTasks: [review-circuit-breakers-step-3-multi-level-fallback]
claimIds: []
...
3.  **Tertiary (Dead-Letter Queue):** If writing to the local file _also_ fails, drop the event to prevent the pipeline from crashing.
...
5.  **Modify the Output:** Open `multi-level-fallback.yaml` and replace the entire `output` section with the `fallback` block below.
    ...
    output:
      fallback:
        ...
        # This is attempted after successful enrichment in the pipeline.
        - switch:
            cases:
