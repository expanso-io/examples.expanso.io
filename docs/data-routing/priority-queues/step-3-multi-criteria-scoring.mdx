...
uniqueTasks: [review-priority-queues-step-3-multi-criteria-scoring]
claimIds: []
...
Modify the `mapping` processor to add an `event_type_score` to the authored `priority_score`. In this configuration, `payment.failed` receives more points than `user.login` when the other fields match.
