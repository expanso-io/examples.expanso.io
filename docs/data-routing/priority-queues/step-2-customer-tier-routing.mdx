...
uniqueTasks: [review-priority-queues-step-2-customer-tier-routing]
claimIds: []
...
If you connect Kafka, inspect the topic outputs. In the authored example, the second `WARN` record selects `logs-high` because its customer tier is `enterprise`.
