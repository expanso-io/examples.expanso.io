...
sidebar_position: 5
description: Inspect account-number masking and a deterministic join-key pattern.
...
uniqueTasks:
  [inspect-enterprise-migration-db2-to-bigquery-step-3-mask-account-numbers]
claimIds: []
...
Reduce account-number exposure with an authored mask and deterministic join key. This page compares the mapping with a DataStage-style transformer.
...
**Security boundary:** Tokenization, keying, access, retention, and regulatory controls require a separate design.
...
pipeline:
  processors:
    ...
    # Step 3: Mask account numbers for downstream control design
    - mapping: |
        root = this
...
<div className="margin-top--lg margin-bottom--lg">
  <a
    href="../step-4-categorize-transactions"
