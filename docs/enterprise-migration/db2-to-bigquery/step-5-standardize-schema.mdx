...
uniqueTasks:
  [inspect-enterprise-migration-db2-to-bigquery-step-5-standardize-schema]
claimIds: []
...
- Convert `TRANSACTION_ID` → `transaction_id`
...
**Example convention:** Lowercase, snake_case column names
**Query Simplicity:** `SELECT transaction_id` vs `SELECT "TRANSACTION_ID"`
**Partitioning:** Date partitions are an authored layout choice; measure query and billing behavior against the target workload
...
| `root = ...` (starting fresh)               | Include explicitly mapped fields     |
...
<div className="margin-top--lg margin-bottom--lg">
  <a
    href="../step-6-validate-required-fields"
