...
sidebar_position: 7
description: "Route selected events to example destinations and remove policy-classified fields before egress."
...
- **Selected local archive** → local disk according to the site's evidence and retention policy

And critically: **strip sensitive topology fields before anything crosses the substation boundary.**

## Policy-Based Field Removal

The example removes named fields before its output stage. Security and compliance owners must supply the actual classification, destination policy, and evidence requirements; this field list does not establish NERC CIP compliance.
...
output:
  ...
  # Local archive example; retention and evidence requirements are site-specific
  - output:
      file:
...
An output after the filter can capture only retained events. If policy requires pre-filter evidence, use a separately reviewed branch before deletion and test its durability, access controls, retention, and failure behavior. Neither pattern by itself establishes a complete audit trail or NERC CIP compliance.
...
RTU/PLC → [Modbus TCP]
  → Expanso Edge (at substation)
      ...
      → Filter by policy          # retained fraction depends on input
...
**Result:** Selected records are routed by the configured policy. Measure input/output volume and delivery latency with representative data.

## What's Next?

You've built the four-stage walkthrough. Review the combined reference configuration:

→ **Next Step:** [Complete SCADA Edge Integration Pipeline](/integrations/scada-energy-edge/complete-scada-integration)
