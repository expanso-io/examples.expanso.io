...
sidebar_position: 8
description: "Combined SCADA reference pipeline for adapter-decoded records, filtering, classification, field removal, and routing."
...
This reference combines all four tutorial stages. It accepts line-oriented records from a required protocol adapter; it does not decode Modbus. Benchmark, threat-model, and qualify the full integration before any operational deployment.
...
#
# Performance and retained volume: measure with representative input
# Governance: example fields are stripped before egress; classification is site-specific
...
pipeline:
  processors:
    ...
    # Drops readings inside illustrative tutorial bands
    # Thresholds require site-specific engineering approval
    # ─────────────────────────────────────────────────────────────────────────
    - mapping: |
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # STAGE 4 (pre-output): Strip fields classified by policy
    # This sample list is not a NERC CIP determination
    # ─────────────────────────────────────────────────────────────────────────
    - mapping: |
        root = this.without(
          "bus_topology",
...
# Selected events → Local evidence archive
# ─────────────────────────────────────────────────────────────────────────────
output:
  broker:
    outputs:
      ...
      # Local evidence archive after filtering; retention is site-specific
      - file:
...
# Illustrative fault thresholds; replace with approved site values
...
- Parsing adapter-decoded line records into structured fields
- Filtering and classifying records before destination routing
- Removing a configured list of fields before external outputs
- Writing retained events to a local evidence file

It does not establish throughput, reduction, alert-latency, reliability, audit-completeness, or NERC CIP compliance claims. Test each of those properties against an approved operating envelope.
