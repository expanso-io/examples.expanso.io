...
RTUs and PLCs may expose data through protocols such as Modbus. This tutorial assumes a separate protocol adapter has already decoded that traffic and emitted a synthetic line record like this:
...
None of that engineering context exists in the adapter-emitted record. This step creates it; the pipeline does not decode a raw Modbus frame.
...
:::tip Policy-Classified Fields
Your security and compliance owners may classify some decoded fields as sensitive. Mark the fields they identify and handle deletion in the routing stage. The sample register below is illustrative, not a NERC CIP determination:
...
You now have structured JSON from adapter-decoded records. Next: apply an example selection policy and measure which records reach the output.

→ **Next Step:** [Step 2: Filter Nominal Readings at the Edge](/integrations/scada-energy-edge/step-2-filter-normalize-readings)
