---
title: "O-RAN Edge Telemetry Pipeline"
sidebar_label: "Overview"
sidebar_position: 1
description: "Transform sample O-RAN telemetry and route it to multiple example destinations from an edge pipeline."
keywords: [oran, telecom, telemetry, edge-processing, kafka, parquet, grafana]
---
...
- **The edge gap**: Data may need to be collected, shaped, and enriched before it reaches Kafka, Grafana, or analytics backends

The result can be unnecessary network use and inconsistent schemas across destinations. Measure the effect with representative telemetry.
...
Tower → Expanso Edge (collect, shape, augment) → Kafka / Cloudera / Grafana / S3
        ...
        └── deployment mechanism must be verified for your OpenShift environment
```

### Key Benefits

- **Edge-native processing**: Collect and transform at the source, not in the datacenter
- **Configurable bandwidth use**: Measure bytes before and after the selected transformations
...
- **OpenShift deployment concept**: This tutorial does not establish operator certification or a supported Red Hat integration

## What You'll Build

This guide walks through an illustrative O-RAN telemetry pipeline that:
...
### [Interactive Explorer](/integrations/oran-telco-pipeline/explorer)
See each O-RAN telemetry processing technique with side-by-side transformations

### [Step-by-Step Tutorial](/integrations/oran-telco-pipeline/setup)
Build the pipeline incrementally:
1. [Collect O-RAN Metrics](/integrations/oran-telco-pipeline/step-1-collect-oran-metrics)
2. [Transform and Enrich](/integrations/oran-telco-pipeline/step-2-transform-and-enrich)
3. [Multi-Destination Routing](/integrations/oran-telco-pipeline/step-3-multi-destination-routing)
4. [Grafana Dashboards](/integrations/oran-telco-pipeline/step-4-grafana-dashboards)
5. [Parquet and Cloudera](/integrations/oran-telco-pipeline/step-5-parquet-and-cloudera)

### [Complete Pipeline](/integrations/oran-telco-pipeline/complete-oran-pipeline)
Review the combined reference configuration
