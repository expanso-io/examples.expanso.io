---
title: 'Step 2: Transform and Enrich O-RAN Telemetry'
sidebar_label: 'Step 2: Transform'
sidebar_position: 5
description: 'Normalize sample O-RAN telemetry and add illustrative site and health metadata.'
keywords: [oran, telecom, telemetry, transformation, enrichment, bloblang]
contentArchetype: step
executionStatus: architecture-only
operationalEvidence: not-assessed
localNavigation: true
uniqueTasks:
  [inspect-integrations-oran-telco-pipeline-step-2-transform-and-enrich]
claimIds: []
...
Use Bloblang to process authored telemetry, adding derived fields, review labels, and site metadata before routing.
...
- **Timing review**: Apply authored timing bands
...
# Illustrative PTP offset bands for this configuration
# Validate bands against the applicable equipment and network requirements
        ...
        root.ptp_review = match {
$abs_offset < 100      => "within_band"  # Within the narrowest example band
$abs_offset < 1000     => "degraded"     # Example warning band
$abs_offset < 10000    => "critical"     # Example critical band
_                      => "outside_band" # Outside the authored bands
        }
        ...
        # Alert level
        root.ptp_alert_level = match this.ptp_review {
          "within_band"     => "none"
          "degraded"      => "warning"
          "critical"      => "major"
          "outside_band" => "critical"
        }
```
...
Add explicit validation checks for transformed data:
...
With data transformed and enriched, proceed to [multi-destination routing](/integrations/oran-telco-pipeline/step-3-multi-destination-routing.mdx) to send processed telemetry to all your observability and analytics platforms.
