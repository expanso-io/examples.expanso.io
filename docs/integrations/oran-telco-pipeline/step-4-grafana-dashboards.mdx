---
title: 'Step 4: Visualize O-RAN Telemetry in Grafana'
sidebar_label: 'Step 4: Grafana'
sidebar_position: 7
description: "Build illustrative Grafana dashboards from the walkthrough's telemetry fields."
keywords: [oran, telecom, telemetry, grafana, dashboards, observability]
contentArchetype: step
executionStatus: architecture-only
operationalEvidence: not-assessed
localNavigation: true
uniqueTasks:
  [inspect-integrations-oran-telco-pipeline-step-4-grafana-dashboards]
claimIds: []
...
Inspect authored Grafana panels for telemetry review. Grafana remains an external service; this architecture walkthrough does not provision it or verify live telemetry.

## Work through this step

1. Configure a Prometheus data source in Grafana and confirm it exposes the `oran_*` metrics used below.
2. Save the three dashboard JSON objects as `dashboards/oran-operations.json`, `dashboards/oran-ptp-review.json`, and `dashboards/oran-performance.json`, respectively.
3. Set `GRAFANA_URL` and `GRAFANA_TOKEN` in your environment, then run the [import script](#dashboard-import-script).
4. Open each imported dashboard, select the data source, and check the time range. A blank panel can mean a missing metric or label; it does not establish that the network is healthy.

The full configurations are expandable below. Start with the operations dashboard; add the timing and performance panels after confirming the metric names and units.
...
set -euo pipefail
: "${GRAFANA_URL:?Set the URL of your Grafana instance}"
: "${GRAFANA_TOKEN:?Set a Grafana token with dashboard write access}"

# Check every input before importing any dashboard.
for file in oran-operations oran-ptp-review oran-performance; do
  test -r "dashboards/${file}.json" || {
    echo "Missing dashboard: dashboards/${file}.json" >&2
    exit 1
  }
done
...
for file in oran-operations oran-ptp-review oran-performance; do
  curl --fail-with-body --silent --show-error \
    ...
    --data-binary "@dashboards/${file}.json" \
...
After checking the imported dashboards against your metrics, proceed to [Parquet storage and Cloudera integration](/integrations/oran-telco-pipeline/step-5-parquet-and-cloudera.mdx) for long-term analytics capabilities.
