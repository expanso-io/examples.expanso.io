---
title: 'Step 5: Store O-RAN Telemetry and Route to Cloudera'
sidebar_label: 'Step 5: Parquet and Cloudera'
sidebar_position: 8
description: 'Configure example Parquet storage and a Kafka path for Cloudera analytics.'
keywords: [oran, telecom, telemetry, parquet, cloudera, kafka, analytics]
contentArchetype: step
executionStatus: architecture-only
operationalEvidence: not-assessed
localNavigation: true
uniqueTasks:
  [inspect-integrations-oran-telco-pipeline-step-5-parquet-and-cloudera]
claimIds: []
...
Review the Parquet conversion and Cloudera integration separately. These are architecture examples; deployment, credentials, and performance validation remain environment-specific.

## Work through this step

1. Choose the partition layout using representative queries. Confirm that input records contain the fields required by the converter schema.
2. Inspect the [conversion service](#parquet-conversion-service), then test it with a small JSON Lines file before processing a batch.
3. Read the generated Parquet files back and compare row counts, partition keys, timestamps, and numeric ranges with the source. Keep source files until that validation is complete.
4. Configure the [Cloudera destination](#cloudera-data-platform-integration) only after the local conversion path is verified.

Expand the configurations and service code as needed. A successful local conversion does not verify delivery to Cloudera.
...
This example organizes Parquet files by authored dimensions. Validate the layout with representative queries:
...
<details>
<summary style={{ minHeight: 44, paddingBlock: 12 }}>Show full YAML configuration</summary>
    ...
    processors:
      ...
      - mapping: |
          ...
          root = {
            ...
            "ptp_review_enum": match this.ptp_review {
              "within_band" => 0,
              "degraded" => 1, 
              "critical" => 2,
              "outside_band" => 3,
...
This illustrative converter expects flattened records matching the schema below. It uses pandas and PyArrow. It splits mixed input into separate partition directories and preserves source files by default. If a batch fails after writing some partitions, remove or reconcile those outputs before retrying; this sample is not an idempotent ingestion service.

<details>
<summary style={{ minHeight: 44, paddingBlock: 12 }}>Show full conversion service</summary>
...
class ORANParquetConverter:
    ...
    def convert_batch(self, input_pattern: str) -> None:
        ...
        for file_path in glob.glob(input_pattern):
            try:
                ...
                # One input file can span several dates, regions, or base stations.
                # Write each partition separately instead of labeling every row
                # with the first record's partition keys.
                partition_keys = ['partition_year', 'partition_month', 'partition_day', 'region', 'gnb_id']
                if df.empty:
                    raise ValueError("Input contains no telemetry rows")
                if df[partition_keys].isna().any().any():
                    raise ValueError("Partition keys must not be null")
                for _, partition in df.groupby(partition_keys, observed=True, sort=False):
                    table = pa.Table.from_pandas(partition, schema=self.schema, preserve_index=False)
                    ...
                    print(f"Converted {file_path} partition to {output_file}")
                ...
                # Preserve the source and make batch failure visible to callers.
                raise RuntimeError(f"Error converting {file_path}: {e}") from e

    def _optimize_datatypes(self, df: pd.DataFrame) -> pd.DataFrame:
        """Optimize pandas datatypes for memory efficiency"""
        # Preserve numeric values until Arrow applies its checked schema cast.
        # pandas astype('int8') can wrap out-of-range integers or truncate
        # fractions before Arrow has a chance to reject them.
    ...
    def _generate_output_path(self, table: pa.Table) -> str:
        ...
        partition_path = os.path.join(
            ...
            f"region={quote(str(first_row['region']), safe='')}",
            f"gnb_id={quote(str(first_row['gnb_id']), safe='')}"
        )

        os.makedirs(partition_path, exist_ok=True)

        # Generate unique filename
        # Multiple batches can finish in the same second in one process.
        filename = f"oran_telemetry_{uuid4().hex}.parquet"
...
compression: 'lz4' # Example codec selection
      batch_size: 65536 # 64KB batch size
      linger_ms: 100 # Allow batching for efficiency
      acks: 'all' # Durability for analytics
      idempotent: true # Producer-level duplicate protection; end-to-end delivery semantics depend on broker and consumer configuration
...
<details>
<summary style={{ minHeight: 44, paddingBlock: 12 }}>Show full JSON configuration</summary>
...
class ORANAnalytics:
    ...
    def calculate_kpis(self, df):
        ...
        return df.groupBy("region", "gnb_id", window("timestamp", "1 hour")) \
            .agg(
                avg("ptp_score").alias("avg_ptp_review"),
    ...
    def generate_daily_report(self, date: str):
        ...
        health_summary = df.agg(
            ...
            count(when(col("ptp_review_enum") == 0, 1)).alias("within_band_sites"),
...
With Parquet storage and Cloudera integration configured, proceed to the [complete pipeline configuration](/integrations/oran-telco-pipeline/complete-oran-pipeline.mdx) to see the full complete example YAML.
