---
title: 'Step 3: Route O-RAN Telemetry to Multiple Destinations'
sidebar_label: 'Step 3: Route Outputs'
sidebar_position: 6
description: 'Fan out transformed telemetry to example dashboard, storage, analytics, and buffering destinations.'
keywords: [oran, telecom, telemetry, routing, fan-out, kafka, parquet]
contentArchetype: step
executionStatus: architecture-only
operationalEvidence: not-assessed
localNavigation: true
uniqueTasks:
  [inspect-integrations-oran-telco-pipeline-step-3-multi-destination-routing]
claimIds: []
...
- **Shared source mapping**: Each destination begins from the same mapped record
            ...
            "resourceMetrics": [{
              "resource": {
                "attributes": [
                  ...
                  {"key": "ptp_review", "value": {"stringValue": this.ptp_review}}
                ]
              },
              "scopeMetrics": [{
                ...
                "metrics": [
                  ...
                  {
                    "name": "oran_ptp_review_score",
                    "description": "PTP review score (0-100)",
                    "unit": "1",
                    "gauge": {
                      "dataPoints": [{
                        "timeUnixNano": this.timestamp_ns.string(),
...
# Define the schema fields used by this example
    schema = pa.schema([
    ...
    for file_path in glob.glob(input_pattern):
        ...
        pq.write_to_dataset(
            ...
            compression='snappy',
            use_dictionary=['du_id', 'cell_id', 'ptp_review']
        )
```
      ...
      - mapping: |
          ...
          root = {
            ...
            "measurements": {
              "timing": {
                "ptp_offset_ns": this.ptp4l_offset_ns,
                "review_status": this.ptp_review,
                "review_score": this.ptp_score,
...
With multi-destination routing configured, proceed to [building Grafana dashboards](/integrations/oran-telco-pipeline/step-4-grafana-dashboards.mdx) to visualize your O-RAN telemetry in real-time.
