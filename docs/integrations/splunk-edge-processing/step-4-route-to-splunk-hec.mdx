...
uniqueTasks:
  [inspect-integrations-splunk-edge-processing-step-4-route-to-splunk-hec]
claimIds: []
...
Now that the example filtering policy is in place, send the retained data to Splunk. This step uses Splunk's HTTP Event Collector (HEC); choose between HEC and other supported ingestion paths based on your deployment requirements.

## outputs.conf and HEC Configuration Examples
...
- Two `tcpout` groups use fixed server lists.
- Index selection is configured outside this `outputs.conf` excerpt.
- The transport uses port 9997 in the example.
- Acknowledgement and compression are enabled.

### Expanso HEC Output

```yaml
# Route on configured conditions
output:
  switch:
...
- Conditions route records by priority, sourcetype, and target index.
- HTTP requests use the configured HEC URL and headers.
- The YAML sets request timeouts and retry behavior.
- A separate branch handles records targeted at the security index.
...
# Multiple outputs with conditional routing
output:
...
# Secondary: Example audit-event file
  - file:
...
Your filtered sample data is now routed to Splunk via HEC. Measure destination bytes and verify retained events before drawing conclusions about cost or data integrity. Next, explore multi-destination routing, policy controls, and metrics extraction.

→ **Next Step:** [Step 5: Advanced Splunk Patterns](/integrations/splunk-edge-processing/step-5-advanced-splunk-patterns)

---

**Key Takeaway:** This example uses HEC with conditional routing and separate destinations for different event types.
