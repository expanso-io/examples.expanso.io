...
description: 'Prepare, filter, and route edge data to Splunk HEC with an independent technical example'
...
This is an independent technical walkthrough, not a Splunk or Cisco partner announcement, certification claim, or product roadmap. Verify current Splunk capabilities, supported ingestion paths, licensing, and pricing with official documentation and your agreement.
:::
...
Many operational datasets originate outside traditional data centers, including:
...
✅ **Adapter-mediated operational sources** — use an appropriate gateway or protocol adapter for OPC UA, Modbus, and proprietary historians, then send a supported stream such as structured records, files, HTTP, or MQTT into the pipeline
✅ **Proprietary telemetry → reviewed event schemas** — map raw readings toward your chosen Splunk fields and validate any CIM claim against the applicable data model
✅ **Smart filtering at source** — apply a reviewed policy to raw telemetry and send the measurements or summaries each use case requires
...
### Win #2: Preparing Data for Analytics and ML

Analytics and ML workflows benefit from consistent schemas, lineage, and quality checks before training or inference.
...
✅ **Semantic enrichment at the source** — attach reviewed source, business, classification, and jurisdiction metadata before routing; downstream ML readiness still requires validation
✅ **Schema normalization across varied sources** — map source-specific fields into a reviewed common schema before events reach the index
✅ **Continuous data quality checks** — enforce schemas, validate field types, and route failed records for review; these checks support but do not guarantee data quality

**Training-data extraction pattern** — the upstream layer can fork a stream. One copy goes to Splunk for analytics. Another reviewed copy can go to a data lake in a format such as Parquet. Data minimization, labels, quality, lineage, and authorization still require workload-specific validation.
...
### Architecture Concept: Querying Data at the Edge

The following section is a conceptual extension, not a claim that Splunk Federated Search currently queries Expanso edge nodes. Implementing it would require a supported connector, identity and authorization model, query semantics, and operating envelope.

Not everything needs to be ingested. Not everything needs to be in S3. Some data is most valuable _in situ_ — sitting on the device that generated it, queryable on demand.
...
The concept brings analytics closer to stored edge data while presenting a unified query surface. It is included for architectural discussion only; the tutorial does not implement that query surface.
...
| **Edge concept** | **On the endpoint** | **Requires a supported connector; not implemented by this tutorial** |
...
For data you're _already_ sending to Splunk, Expanso can transform it before HEC ingestion. Measure quality, latency, volume, and cost in your environment:

✅ **Pre-parsed fields** — may reduce search-time extraction work; benchmark representative searches
...
### [Interactive Explorer](/integrations/splunk-edge-processing/explorer)

See each Splunk integration technique with side-by-side transformations

### [Step-by-Step Tutorial](/integrations/splunk-edge-processing/setup)

Build the pipeline incrementally:

1. [Collect Like inputs.conf](/integrations/splunk-edge-processing/step-1-collect-like-inputs-conf)
2. [Parse Like props.conf](/integrations/splunk-edge-processing/step-2-parse-like-props-conf)
3. [Filter Before Indexing](/integrations/splunk-edge-processing/step-3-filter-before-indexing)
4. [Route to Splunk HEC](/integrations/splunk-edge-processing/step-4-route-to-splunk-hec)
5. [Advanced Splunk Patterns](/integrations/splunk-edge-processing/step-5-advanced-splunk-patterns)

### [Complete Pipeline](/integrations/splunk-edge-processing/complete-splunk-integration)

Review the complete reference pipeline

---

_Use this example to evaluate edge preparation and HEC routing against your own workload, controls, and commercial terms._
