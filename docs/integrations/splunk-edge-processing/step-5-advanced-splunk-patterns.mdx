...
description: 'Inspect multi-destination routing, data classification, metrics extraction, and adaptive processing.'
contentArchetype: step
executionStatus: architecture-only
operationalEvidence: not-assessed
localNavigation: true
uniqueTasks:
  [inspect-integrations-splunk-edge-processing-step-5-advanced-splunk-patterns]
claimIds: []
...
Inspect optional routing and transformation branches around a Splunk destination. Their operational and commercial effects require workload-specific testing.
...
metadata:
  ...
  description: "Advanced pipeline with multi-destination routing and review features"
...
processors:
  ...
  # 2. Enhanced parsing with review fields
  - conditional:
  ...
  - mapping: |
      ...
      root.splunk_event = {
        ...
        "index": match {
          this.data_classification == "pii" || this.data_classification == "financial" => "review"
...
output:
  ...
  # Secondary: S3 for long-term storage and review
...
cat >> ~/advanced-splunk-pipeline.yaml << 'EOF'
  ...
  - mapping: |
      ...
      root.filter_aggressiveness = match {
        ...
        this.data_classification == "pii" => "low"  # Use the least aggressive example sampling branch
        _ => "normal"
      }
...
Route data from an explicit geographic policy:
...
output:
  # Route based on geographic review
  switch:
...
```bash
# Generate PII event (should go to review index + S3)
...
```spl
# Check review data
index=review pii_masked=true earliest=-15m
...
| **Pattern**             | **What this page configures**                | **Boundary to validate**                           |
| ----------------------- | -------------------------------------------- | -------------------------------------------------- |
| **Multi-Destination**   | Separate output branches                     | Delivery behavior for every destination            |
| **Adaptive Processing** | Conditions that select authored rules        | Workload behavior and failure handling             |
| **Anomaly Fields**      | Example thresholds and alert-shaped records  | Detection quality and end-to-end timing            |
| **Geographic Routing**  | Destination labels selected from record data | Legal basis and destination policy remain external |
| **Sampling**            | Deterministic example sampling branches      | Distribution and downstream analysis               |
...
```spl
# Report delivered event and byte volume; apply current contract pricing separately
...
| table events_sent, gb_sent, edge_nodes
```

## What's Next?

You have now implemented several optional routing and policy patterns. Next, review the combined reference pipeline and decide which operational controls your environment still needs.

→ **Next Step:** [Complete Production Pipeline](/integrations/splunk-edge-processing/complete-splunk-integration)

---

**Key takeaway:** These patterns can complement a Splunk deployment with upstream routing and transformation. Feature parity, compatibility, regulatory outcomes, and commercial outcomes require workload-specific testing and review.
