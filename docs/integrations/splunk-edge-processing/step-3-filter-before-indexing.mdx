---
title: "Step 3: Filter Before Indexing"
description: "Apply measurable edge-filtering policies before sending selected events to Splunk"
---

# Step 3: Filter Before Indexing

This stage applies an explicit filtering policy before events reach Splunk HEC. Whether that reduces billable ingest or total cost depends on your Splunk product, contract, workload, retention policy, and the events you choose to keep.

:::caution Measure before making cost claims
The sample policy intentionally drops and samples data. Test it against representative traffic, verify that required security and audit events are preserved, measure bytes accepted by the destination, and apply your own contracted pricing. This tutorial does not claim a typical reduction or savings percentage.
:::

## Build a Baseline

Classify a representative sample using your retention and investigation requirements. Useful baseline categories include verbose diagnostics, repetitive health checks, duplicate events, routine operational events, and security or business records that must be retained.

## Source Filtering and Search Filtering Are Different Controls

The following SPL narrows search results after ingestion. The edge policy later in this step changes what is sent to HEC. Treat those as different controls with different failure modes:
...
metadata:
  ...
  description: "Apply an example filtering policy before events reach Splunk"
...
- ✅ **1 health check** out of the 10 synthetic checks (the configured sample behavior)
...
| **Filter Type** | **SPL Equivalent** | **Expanso Edge Filtering** | **Measure** |
|---|---|---|---|
| **Level Filtering** | `level!="DEBUG"` | `root = deleted()` if DEBUG | Events and bytes removed by level |
| **Health Check Sampling** | `NOT (message="health*")` | Keep 1 in 10 via hash | Sample accuracy and retained failures |
| **Deduplication** | `dedup user,message` | Deduplication processor | Duplicate window and false matches |
| **Test Data Removal** | `user!="test*"` | Conditional deletion | Matches reviewed before deletion |
| **Rate Limiting** | Manual time windows | Throttling policy | Delayed or dropped events under load |
...
Use measured daily destination bytes and your contracted unit to build the comparison:

```text
baseline_cost = priced_baseline_volume × contracted_rate
filtered_cost = priced_filtered_volume × contracted_rate
net_effect = baseline_cost - filtered_cost - added_edge_and_operating_costs
```

Include infrastructure, support, operations, retention effects, and the value or risk of data removed. Do not extrapolate the synthetic event mix in this tutorial to a production workload.
...
You now have a configurable pre-ingest policy and a way to measure its behavior. Next, configure the output to send the retained events to Splunk via HEC.

→ **Next Step:** [Step 4: Route to Splunk HEC](/integrations/splunk-edge-processing/step-4-route-to-splunk-hec)

---

**Key takeaway:** Pre-ingest filtering can change destination volume, but the policy must be validated against investigation, security, legal, and operational requirements before deployment.
