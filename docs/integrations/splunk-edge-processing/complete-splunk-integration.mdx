---
title: "Complete Reference Pipeline"
description: "Combined Splunk HEC reference pipeline with example filtering, routing, and monitoring controls"
---
...
This pipeline combines the tutorial concepts into a single reference configuration. It has not been qualified for your traffic, availability target, retention obligations, Splunk contract, or security environment; validate and harden it before deployment.

<CopyPipelineButton yamlUrl="/pipelines/splunk-production-pipeline.yaml" />

## Production Pipeline Configuration

```yaml
# production-splunk-pipeline.yaml
# Combined Expanso Edge reference pipeline for Splunk HEC integration
...
metadata:
  ...
  description: "Reference Splunk HEC integration with configurable edge filtering"
  version: "1.0.0"
  labels:
...
Need a recovery path or rollout checklist? Start with the main [Splunk edge processing guide](/integrations/splunk-edge-processing/) and the [setup walkthrough](/integrations/splunk-edge-processing/setup/) to validate inputs, parsing, filtering, and routing stage by stage.

---

**This combined reference pipeline demonstrates how to:**
- Apply configurable filtering before HEC ingestion
- Test retention of the events your policy defines as critical
- Mask selected fields as one component of a broader privacy and security program
- ✅ Provide multi-destination routing
- ✅ Enable real-time alerting
- ✅ Support long-term analytics via S3

Measure accepted destination volume, event retention, delivery behavior, query performance, and total operating cost before making production or savings claims.
