...
uniqueTasks:
  [inspect-integrations-splunk-edge-processing-step-2-parse-like-props-conf]
claimIds: []
...
In Splunk, `props.conf` and `transforms.conf` handle parsing and field extraction. This example uses the **Bloblang** mapping language to perform those operations at collection time.

## Splunk Parsing Configuration Example

### props.conf + transforms.conf Setup

The Splunk example uses these `props.conf` and `transforms.conf` entries:
...
## Expanso Bloblang Mapping

In this example, parsing happens **at collection time** with the following mapping:
...
metadata:
  ...
  description: "Parse data like props.conf during collection"
...
## Parsing Locations Represented in This Tutorial

| **Operation**        | **Splunk snippet**        | **Pipeline snippet**        |
| -------------------- | ------------------------- | --------------------------- |
| **Field extraction** | SPL command during search | Bloblang mapping before HEC |
| **Parsing point**    | During search             | During pipeline processing  |
| **Output shape**     | Search fields             | Structured JSON             |
...
The pipeline now parses data at collection time. Next, implement filtering before data reaches the indexers.

→ **Next Step:** [Step 3: Filter Before Indexing](/integrations/splunk-edge-processing/step-3-filter-before-indexing)

---

**Key takeaway:** Bloblang performs the example's parsing before HEC ingestion, while SPL is used here to illustrate analogous field extraction. Compare correctness, maintainability, and performance with your actual workload.
