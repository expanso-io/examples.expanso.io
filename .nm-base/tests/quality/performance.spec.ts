...
async function recordTrustedExplorerInteraction(
  explorer: Locator,
  index: number
): Promise<void> {
  const compactSelector = explorer.getByRole('combobox', {
  ...
  if (await compactSelector.isVisible()) {
    ...
    await compactSelector.press('Home');
    for (let step = 0; step < index; step += 1) {
      await compactSelector.press('ArrowDown');
    }
    await compactSelector.press('Enter');
    return;
  }
  await explorer
    .getByRole('button', { name: new RegExp(`Stage ${index + 1} of`) })
...
test('collects exact-SHA performance evidence from the production artifact', async ({
  ...
  await explorerPage.addInitScript(() => {
    ...
    const record = (list: PerformanceObserverEntryList) => {
    ...
    new PerformanceObserver(record).observe({
      type: 'event',
      buffered: true,
      durationThreshold: 0,
    });
  ...
  await recordTrustedExplorerInteraction(explorer, 1);
