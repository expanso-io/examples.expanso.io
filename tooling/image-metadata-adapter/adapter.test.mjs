...
const { imageSize, MAX_BYTES } = fromLoader('image-size');
test('Docusaurus resolves the replacement and original vulnerable parser exports are absent', () => {
  assert.match(
    fromLoader.resolve('image-size/fromFile'),
    /tooling\/image-metadata-adapter\/from-file.cjs$/
  );
  for (const type of ['icns', 'jxl', 'heif'])
    assert.throws(() => fromLoader.resolve('image-size/types/' + type));
});
test('standard web image headers retain dimensions', () => {
  ...
  assert.deepEqual(
    [
      imageSize(Buffer.from('<svg width="32" height="24"></svg>')).width,
      imageSize(Buffer.from('<svg viewBox="0 0 32 24"></svg>')).height,
    ],
    [32, 24]
  );
});
test('all checked-in web images are supported by the replacement', async () => {
  let count = 0;
  for (const entry of await readdir('static', {
    recursive: true,
    withFileTypes: true,
  })) {
    if (!entry.isFile() || !/\.(png|jpe?g|gif|svg|webp)$/i.test(entry.name))
      continue;
  ...
  assert.ok(
    count >= 3,
    'the docs logo and favicon inventory must be exercised'
  );
});
test('zero-sized ICNS, JXL and HEIF boxes fail promptly rather than hanging the build', () => {
  const samples = [
    '69636e73000000106963703000000000',
    '0000000c4a584c200d0a870a000000006a786c63',
...
test('oversized data and malformed dimensions fail closed', () => {
  assert.throws(
    () => imageSize(Buffer.alloc(16 * 1024 * 1024 + 1)),
    /oversized/
  );
  assert.throws(
    () => imageSize(Buffer.from('<svg width="0" height="0"></svg>')),
    /Invalid/
  );
});

test('invalid and oversized inputs are rejected before allocation or coercion', (t) => {
  let coerced = false;
  const invalid = {
    valueOf() {
      coerced = true;
      return '<svg width="32" height="24"></svg>';
    },
  };
  const oversized = new Uint8Array(MAX_BYTES + 1);
  const from = t.mock.method(Buffer, 'from', () => {
    throw new Error('Buffer allocation must not be reached');
  });
  for (const input of [
    oversized,
    invalid,
    ...
    assert.throws(() => imageSize(input), /Unsupported or oversized web image/);
  }
