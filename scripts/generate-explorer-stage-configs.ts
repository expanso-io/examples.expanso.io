...
const GENERATED_EVIDENCE_MODULE_PATH =
  'src/catalog/explorerEvidence.generated.ts';
const GENERATED_FAMILY_MODULE_ROOT =
...
type SourceStage = Omit<Stage, 'slug'> & { slug?: string };
...
async function renderGeneratedModules(
  manifest: StageManifest,
  manifestBytes: string,
  configBytes: ReadonlyMap<string, string>,
  pipelineBytes: ReadonlyMap<string, string>,
  sourceStagesById: ReadonlyMap<string, readonly SourceStage[]>
): Promise<{
  configs: string;
  evidence: string;
  ...
  const familyEntries = manifest.explorers.map((explorer) => {
    const record = recordById.get(explorer.exampleId);
    if (!record) {
      throw new Error(`Catalog record is missing for ${explorer.exampleId}`);
    }
    if (!record.completePipelinePath || !record.explorerEvidence) {
      throw new Error(
        `Catalog Explorer binding is incomplete for ${explorer.exampleId}`
      );
    }
    const sourceStages = sourceStagesById.get(explorer.exampleId);
    if (!sourceStages || sourceStages.length !== explorer.stages.length) {
      throw new Error(
        `Generated presentation stages are missing for ${explorer.exampleId}`
      );
    }
    const stages = explorer.stages.map((stage, index) => {
      ...
      return {
        ...
        inputLines: sourceStage.inputLines,
        outputLines: sourceStage.outputLines,
        ...(sourceStage.inputFormat
          ? { inputFormat: sourceStage.inputFormat }
          : {}),
        ...(sourceStage.outputFormat
          ? { outputFormat: sourceStage.outputFormat }
          : {}),
    ...
    return {
      exampleId: explorer.exampleId,
      family: {
        binding: {
          exampleId: record.id,
          canonicalPipelinePath: record.completePipelinePath,
          provenance: record.explorerEvidence.kind,
          executionStatus: record.executionStatus,
          operationalEvidence: record.operationalEvidence,
          fixtureLabel: basename(record.explorerEvidence.inputCheckpointPath),
          comparisonMode:
            record.explorerEvidence.kind === 'curated-explanation' &&
  ...
  const entries = familyEntries.map(
    ({ exampleId, family }) =>
      `  ${JSON.stringify(exampleId)}: ${JSON.stringify(family)},`
  );
...
`,
  ...
  const families = new Map(
    await Promise.all(
      familyEntries.map(async ({ exampleId, family }) => {
        const { stages, ...metadata } = family;
        return [
          exampleId,
          await format(
            `/** Generated by scripts/generate-explorer-stage-configs.ts. */
...
export const GENERATED_EXPLORER_STAGES = ${JSON.stringify(stages)} satisfies readonly GeneratedExplorerStageConfig[];
...
`,
  ...
  return { configs, evidence, families };
}

function writeGeneratedModules(generated: {
  configs: string;
  evidence: string;
  families: ReadonlyMap<string, string>;
}): void {
  writeFileSync(GENERATED_MODULE_PATH, generated.configs);
  writeFileSync(GENERATED_EVIDENCE_MODULE_PATH, generated.evidence);
  mkdirSync(GENERATED_FAMILY_MODULE_ROOT, { recursive: true });
  for (const [exampleId, source] of generated.families) {
    writeFileSync(`${GENERATED_FAMILY_MODULE_ROOT}/${exampleId}.ts`, source);
  }
}
...
async function bootstrap(): Promise<void> {
  ...
  const sourceStagesById = new Map<string, readonly SourceStage[]>();
  ...
  for (const record of EXAMPLE_RECORDS.filter(({ routes }) => routes.explore)) {
    ...
    sourceStagesById.set(record.id, stages);
  ...
  const generated = await renderGeneratedModules(
    manifest,
  ...
  writeGeneratedModules(generated);
}

async function validateAndRender(manifest: StageManifest): Promise<{
  generated: {
    configs: string;
    evidence: string;
    families: ReadonlyMap<string, string>;
  ...
  for (const [recordIndex, record] of catalogExplorers.entries()) {
    ...
    if (!existsSync(binding.stageModule)) {
    ...
    pipelineBytes.set(record.completePipelinePath, canonicalPipelineBytes);
    let loadedStages: SourceStage[];
    try {
      loadedStages = await loadExplorerStages(record.id, binding);
    } catch (error) {
      throw new Error(
        `${record.id} stage module binding is stale: ${error instanceof Error ? error.message : String(error)}`
      );
    }
    const sourceStages = validateStageIdentity(loadedStages, record.id, true);
    sourceStagesById.set(record.id, sourceStages);
  ...
  return {
    generated: await renderGeneratedModules(
      manifest,
...
async function main(): Promise<void> {
  ...
  if (writeMode || bootstrapMode) {
    ...
    for (const [path, expected] of [
      [GENERATED_MODULE_PATH, result.generated.configs],
      [GENERATED_EVIDENCE_MODULE_PATH, result.generated.evidence],
    ] as const) {
      if (!existsSync(path) || readFileSync(path, 'utf8') !== expected) {
        throw new Error(`${path} is stale; run npm run stages:canonical:write`);
      }
    }
    for (const [exampleId, expected] of result.generated.families) {
      const path = `${GENERATED_FAMILY_MODULE_ROOT}/${exampleId}.ts`;
      if (!existsSync(path) || readFileSync(path, 'utf8') !== expected) {
        throw new Error(`${path} is stale; run npm run stages:canonical:write`);
      }
    }
