...
const guardedChallenge = legacyChallenge.replace(
...
describe('production canary Cloudflare normalization', () => {
  ...
  for (const [name, challenge] of [
    ...
    ['guarded contentWindow', guardedChallenge],
  ] as const) {
    it(`strictly removes the ${name} Cloudflare JS-detection tail`, () => {
      const result = verifyCanaryEntry({
        expected,
        expectedBytes: expectedHtml,
  ...
  it('rejects unrecognized production drift', () => {
    const observed = Buffer.from(
      `<html><body>tampered<script src="/other.js"></script>${guardedChallenge}</body></html>`
    );
    assert.throws(
      () =>
        verifyCanaryEntry({
  ...
  it('rejects a lookalike Cloudflare challenge', () => {
    const observed = Buffer.from(
      `<html><body>support@example.com${guardedChallenge.replace('/jsd/main.js', '/jsd/other.js')}</body></html>`
    );
    assert.throws(
  ...
  it('rejects arbitrary document-lookup variations', () => {
    const observed = Buffer.from(
      `<html><body>support@example.com${guardedChallenge.replace(
        'a.contentDocument||(a.contentWindow&&a.contentWindow.document)',
        'a.contentDocument||(a.contentWindow?.document)'
      )}</body></html>`
    );
