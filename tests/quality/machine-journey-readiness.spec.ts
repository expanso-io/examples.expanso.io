...
test.describe('machine journey browser readiness', () => {
  ...
  test('keeps the customer problem, solution, and Explorer on one page', async ({
    page,
  }) => {
    await page.goto(overview, { waitUntil: 'networkidle' });
    const main = page.locator('main');
    await expect(
      main.getByRole('heading', { name: 'The problem' })
    ).toBeVisible();
    await expect(
      main.getByRole('heading', { name: 'How Expanso solves it' })
    ).toBeVisible();
    await expect(main.locator('[data-explorer-version="2"]')).toBeVisible();
    await expect(main).not.toContainText('System boundary');
    await expect(main).not.toContainText('Limitations');
  });
